...
sim = PhaseSimulator(oscillation_freq = 10,
                    FS               = 1000,
                    WIN_SIZE         = 100,
                    window_stop      = 500,
                    PROP_LENGTH      = 100,
...
ax.plot(sim.prop_phase[:,0],label='propagated phase')
ax.plot(sim.gtruth[:,0],label='true phase')
ax.plot(sim.ar_phase[:,0],label='ar phase')
ax.axvline(50,color='w',linestyle='--',label='critical time')
ax.legend()
#sim.ar_fit_and_extrapolation()
#sim.get_R(mode="ar")
#sim.get_R(mode="prop")
...
ttest = scipy.stats.ttest_ind(ar_distances, prop_distances)
ttest

# %%
fig, ax = plt.subplots(1,1,figsize=(10,5))
i_trial = 10
ax.plot(sim.prop_phase[:,i_trial],label='FC prop phase')
ax.plot(np.arange(50,100),sim.ar_phase[:,i_trial],label='AR prop phase')
ax.plot(sim.gtruth[:,i_trial],label='Ground truth phase')
ax.axvline(50,color='w',linestyle='--',label='critical time')
ax.legend()
...
ax[0].plot(A)
ax[0].set_title("AR Kernel")
ax[1].plot(exdat[:,i_trial])
ax[1].plot(sim.lfp[:,i_trial])
...
plt.acorr(sim.lfp[:,0], maxlags = 100)
